...
from scrappy_forge.platform_workspace import load_manifest, materialize, status, validate_manifest
from scrappy_forge.util import ForgeError


def test_packaged_manifest_pins_four_components():
    ...
    assert all(len(item["sha"]) == 40 for item in manifest["components"])
...
def test_manifest_refuses_floating_or_short_sha():
    manifest = load_manifest()
    manifest["components"][0]["sha"] = "main"
...
def test_materialize_checks_out_exact_pins(monkeypatch, tmp_path: Path):
    ...
    assert {call[-1] for call in checkouts} == {item["sha"] for item in manifest["components"]}
